...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    **${message.author.username}** you must have fished up to
    \`50\` times to get rewards from here. so continue fishing
    `)
    message.channel.send(nope1);
    } else if (result[0].crime < 50) {
    ...
    **${message.author.username}** you must have worked up to
    \`50\` times to get rewards from here.
      ...
      let con = sql.query("SELECT channelid AS res FROM economy_ward WHERE guildid = " + message.guild.id, function (error, results, fields) {
       if (error) console.log(error);
        ...
        if (!log.guild.member(client.user).hasPermission("EMBED_LINKS", "VIEW_CHANNEL", "READ_MESSAGE_HISTORY", "VIEW_AUDIT_LOG", "SEND_MESSAGES")) return;
          ...
          const daily = ["🍰 You bought up to \`50\` products from the global market\nfor that here's your daily reward of", "🐟 You have caught up to \`50\` fishes\nHere's your daily reward of", "🎭 You have committed up to \`50\` crimes\nVery impressive for that take", "🦺 You have worked up to \`50\` times. a very good woker\nFor that I think you deserve this"];
      ...
      let con = sql.query("SELECT channelid AS res FROM economy_ward WHERE guildid = " + message.guild.id, function (error, results, fields) {
       if (error) console.log(error);
        ...
        if (!log.guild.member(client.user).hasPermission("EMBED_LINKS", "VIEW_CHANNEL", "READ_MESSAGE_HISTORY", "VIEW_AUDIT_LOG", "SEND_MESSAGES")) return;
          ...
          const weekly = ["🍰 You bought up to \`50\` products from the global market\nfor that here's your weelky reward of", "🐟 You have caught up to \`50\` fishes\nHere's your weekly reward of", "🎭 You have committed up to \`50\` crimes\nVery impressive for that take", "🦺 You have worked up to \`50\` times. a very good woker\nFor that I think you deserve this"];
    ...
    if (args[0] === "claimrole") {
    if (message.member.roles.cache.some(role => role.id === '871157647490052096')) {
    let RewardRole = message.member.guild.roles.cache.find(role => role.id === "881642679748939796");
    ...
    // .setDescription(`
    // <:rabbitbullet:887617523925778443> ... **1** - \`daily\`
    // <:rabbitbullet:887617523925778443> ... **2** - \`weekly\`
    // <:rabbitbullet:887617523925778443> ... **3** - \`xpcounter\`
    // <:rabbitbullet:887617523925778443> ... **4** - \`role\`
