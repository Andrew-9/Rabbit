...
module.exports = {
    name: "withdrawsales",
    aliases: ["withs"],
    description: "Withdraw money from your sales account to your balance",
    category: "Economy",
    usage: "withdrawsales <amount>",
            ...
            let con = sql.query("SELECT channelid AS res FROM economy_ward WHERE guildid = " + message.guild.id, function (error, results, fields) {
             if (error) console.log(error);
              ...
              if (!log.guild.member(client.user).hasPermission("EMBED_LINKS", "VIEW_CHANNEL", "READ_MESSAGE_HISTORY", "VIEW_AUDIT_LOG", "SEND_MESSAGES")) return;
                ...
                let b = sql.query("SELECT balance, bank, sales_balance FROM economy WHERE userId = ?;", message.author.id, function (err, result, fields){
                if (Object.keys(result).length === 0) {
                let register ="INSERT INTO economy (userId, balance, bank, sales_balance, username, HasShop, is_jailed, cardtype) VALUES ('" + message.author.id + "', '0', '0', '0',' " + message.author.username + "', '0', '0', '1');";
                sql.query(register)
                ...
                } else if (toFixed(args[0]) <= toFixed(result[0].sales_balance)){
                if (toFixed(args[0]) > 0){
                let rembal = 'UPDATE economy SET balance = balance+' + args[0] + ' WHERE userId = ' + message.author.id + '';
                sql.query(rembal);
                let addbank = 'UPDATE economy SET sales_balance = sales_balance-' + args[0] + ' WHERE userId = ' + message.author.id + '';
                sql.query(addbank);
                ...
                .setDescription('Withdrawn **' + WithdrawAmount + '<:dollars:881559643820793917>** from sales balance!')    
                .setTimestamp()
                .setFooter(message.guild.name, message.guild.iconURL());  
                ...
                let WithdrawAmount = new Intl.NumberFormat("en-US", {style: 'currency', currency: 'USD'}).format(args[0]);
                let TotalBalance = new Intl.NumberFormat("en-US", {style: 'currency', currency: 'USD'}).format(result[0].sales_balance);
