...
module.exports = {
 name: "tempinvite",
 aliases: ["temporary-invite", "cti"],
 description: "Use the bot to create a temporary invite link\nfor your server and the bot will DM it to you.",
 category: "General",
 usage: "tempinvite",
       ...
       if (reason.length === 0) {
        ...
        sql.query(sqlquery, function (error, results, fields) {
         ...
         (async () => {
          ...
          message.channel.createInvite({maxAge: 86400,maxUses: 10,unique: true,})
            .then((invite) => {
              message.lineReply(`<:winkingface_01:869178842957381734> **| Invite link created. Check your DM**`);
              const embedGen = new embGen();
              const tempInvite = embedGen.generatetempInvEmb(client,user,message,reason,dUrl,invite);
              message.author.send(tempInvite);
              log.send(tempInvite);
            })
          });
         })();
