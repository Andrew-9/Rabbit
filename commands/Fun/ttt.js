...
module.exports = {
 ...
 run: async (client, message, args) => {
        ...
        if(!opponent) return message.channel.send("<:xvector:869193619318382602> **| Please provide the user to challenge!**")
        ...
        async function tictactoe(m) {
            ...
            collector.on('collect', b => {
                ...
                if (Object.keys(Brgs).length == 0) return m.edit('**Ohh It\'s a tie!**')
                tictactoe(m)
            });
            collector.on('end', collected => {
                if (collected.size == 0) m.edit(`<:xvector:869193619318382602> **| <@!${Args.userid}> didn\'t react in time! (30s)**`)
            });
        }
    }
}