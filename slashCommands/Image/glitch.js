...
module.exports = {
    ...
    interaction.reply({ content: `<a:is_loading:923892698782511125> **Affecting user's avatar.. Hold On...**`,  components: [row] })
    await wait(2000);
    const buffer = await AmeAPI.generate("glitch", { url: user.displayAvatarURL({ format: "png", size: 2048 }) });
    const attachment = new MessageAttachment(buffer, "glitch.png");
    interaction.editReply({
    content: `**${emoji} Easily use this same command with \`${prefix}glitch\`**`,
