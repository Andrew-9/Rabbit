...
module.exports = {
	name: "playskip",
	description: "Play new song and skip the current song", //the command description for Slash Command Overview
	cooldown: 2,
	...
	run: async (client, interaction) => {
		try {
			...
			//Edit the reply
			interaction.editReply({ content: `${queue?.songs?.length > 0 ? " " + emoji + " **Skipping to:**" : " " + emoji + " **Now Playing:**"} \`${Text}\``, components: [row], });
			await wait(4000);
