...
module.exports = {
	...
	run: async (client, interaction) => {
		try {
			...
			let seekNumber = options.getInteger("position")
			...
			.setDescription(`The Seek Position must be between \`0\` and \`${newQueue.songs[0].duration}\``)
			],
			components: [row],
			ephemeral: true
			});
			...
			.setDescription(`Seeked the song to \`${seekNumber} seconds\``)
			]
			});
			} catch (e) {
