...
module.exports = {
	name: "addrelated",
	description: "Add a similar/related song of the current Song!",
	cooldown: 2,
	...
	run: async (client, interaction) => {
		try {
			...
			await interaction.reply({ content: `${emoji} **Searching related song for... \`${newQueue.songs[0].name}\`**`, 	components: [row] });
			await newQueue.addRelatedSong();
			await interaction.editReply({ content: `${emoji} **Added:** \`${newQueue.songs[newQueue.songs.length - 1].name}\``, components: [row] });
			await wait(5000);
